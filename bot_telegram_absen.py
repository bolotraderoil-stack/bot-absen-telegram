...
def home_genset():
    try:
        ...
        for r in data:
            ...
            row_class = "low-bbm" if is_low else ""
            rows += f"<tr class='{row_class}'><td>{tanggal}</td><td>{mulai.strftime('%H:%M') if mulai else '-'}</td><td>{selesai.strftime('%H:%M') if selesai else '-'}</td><td>{durasi_str}</td><td>{awal}%</td><td>{akhir}%</td><td>{pakai}%</td><td>{sisa}%</td><td>{petugas}</td></tr>"
...
"""
       ...
       .filter{{text-align:center;margin:20px 0;display:flex;flex-wrap:wrap;gap:10px;justify-content:center}}
        input,select,button{{padding:10px 12px;font-size:16px;border-radius:8px;border:1px solid #ddd;flex:1 1 150px}}
       ...
       .table-wrapper{{overflow-x:auto}}
        ...
        tr.low-bbm{{background:#ffebee;color:#c62828;font-weight:bold}}

        /* CARD buat mobile */
       .cards-container{{display:none;gap:12px}}
       .card{{background:white;border-radius:12px;padding:15px;box-shadow:0 2px 6px rgba(0,0,0,0.1);border-left:5px solid #FF9800}}
       .card-low{{border-left:5px solid #c62828}}
       ...
       .card-body p{{margin:8px 0;font-size:15px}}
        ...
        <a href="/export_genset?tanggal={tanggal if tanggal else ''}" style="padding:10px 12px;background:#4CAF50;color:white;text-decoration:none;border-radius:8px;font-weight:bold">⬇️ CSV</a>
        </form></div>
        ...
        const infoDetail = {json.dumps(info_detail)};
        new Chart(ctx, {{
            type: 'line',
            data: {{
            ...
            options: {{
                ...
                plugins: {{
                    title: {{display: true, text: 'Grafik BBM Genset', font: {{size: 16}}}},
