...
def home():
    try:
        ...
        html = """<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Data Absensi</title><meta name="viewport" content="width=device-width, initial-scale=1"><style>body {{ font-family: Arial, sans-serif; padding: 20px; background: #f5f5f5; }}h2 {{ text-align: center; }}table {{ width: 100%; border-collapse: collapse; background: white; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }}th, td {{ padding: 12px; text-align: left; border-bottom: 1px solid #ddd; }}th {{ background: #4CAF50; color: white; }}tr:hover {{ background: #f1f1f1; }}.telat {{ background: #ffebee; color: #c62828; font-weight: bold; }}.status-izin {{ color: orange; }}.status-sakit {{ color: red; }}.status-cuti {{ color: blue; }}.status-lembur {{ color: purple; font-weight: bold; }}.filter {{ text-align: center; margin-bottom: 20px; }}input, button {{ padding: 8px; font-size: 16px; }}@media (max-width: 600px) {{table, thead, tbody, th, td, tr {{ display: block; }}th {{ display: none; }}td {{ border: none; position: relative; padding-left: 50%; }}td:before {{content: attr(data-label);position: absolute;left: 10px;font-weight: bold;}}}}</style></head><body><h2>📋 Data Absensi</h2><div class="filter"><form method="get"><input type="date" name="tanggal" value="{tgl}"><button type="submit">Filter</button><a href="/"><button type="button">Reset</button></a></form></div><table><thead><tr><th>Nama</th><th>Tanggal</th><th>Datang</th><th>Pulang</th><th>Status</th><th>Alasan</th><th>Total Jam</th></tr></thead><tbody>""".format(tgl=tanggal if tanggal else "")
...
def get_keyboard(status):
    buttons = []
    if status == 'belum':
        buttons.append([InlineKeyboardButton("✅ Datang", callback_data='minta_lokasi_datang'), InlineKeyboardButton("🚪 Pulang", callback_data='pulang'), InlineKeyboardButton("📝 Izin", callback_data='izin')])
    elif status in ['datang', 'lembur']:
        buttons.append([InlineKeyboardButton("🚪 Pulang", callback_data='minta_lokasi_pulang'), InlineKeyboardButton("📝 Izin", callback_data='izin'), InlineKeyboardButton("🤒 Sakit", callback_data='sakit')])
...
def simpan_datang(user_id, nama, lat, lon):
    ...
    try:
        cur.execute("INSERT INTO absensi (user_id, nama, tanggal, jam_datang, lat_datang, lon_datang, status, telat, alasan) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT (user_id, tanggal) DO NOTHING", (user_id, nama, hari_ini, jam_sekarang, lat, lon, status, telat, jenis_libur))
        conn.commit()
...
def simpan_pulang(user_id, lat, lon):
    ...
    hari_ini = wib.date()
    jam_sekarang = wib.time()
    cur.execute("UPDATE absensi SET jam_pulang=%s, lat_pulang=%s, lon_pulang=%s WHERE user_id=%s AND tanggal=%s AND jam_datang IS NOT NULL AND jam_pulang IS NULL", (jam_sekarang, lat, lon, user_id, hari_ini))
    conn.commit()
...
def get_rekap_bulanan(user_id, bulan_str=None):
    ...
    if bulan_str: tahun, bulan = map(int, bulan_str.split('-'))
    else: tahun, bulan = now.year, now.month
    ...
    if hari_hadir > 0: rata_detik = total_detik // hari_hadir; rh = rata_detik // 3600; rm = (rata_detik % 3600) // 60; rata_fmt = f"{rh:02d}j {rm:02d}m"
    else: rata_fmt = "00j 00m"
...
async def minta_lokasi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"📍 Kirim lokasi Anda.\nRadius maks {RADIUS_METER}m dari kantor", reply_markup=keyboard)
...
async def terima_lokasi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if jarak > RADIUS_METER:
        await update.message.reply_text(f"❌ Gagal absen. Jarak Anda {int(jarak)} meter dari kantor. Maksimal {RADIUS_METER} meter.", reply_markup=ReplyKeyboardRemove())
...
async def export_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args)!= 1:
        ...
        return
    try: tahun, bulan = map(int, context.args[0].split('-'))
    except:
        await update.message.reply_text("Format salah. Contoh: `/export 2025-10`", reply_markup=get_keyboard(cek_absen(user_id)))
        return
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT tanggal, jam_datang, jam_pulang, status, alasan, EXTRACT(EPOCH FROM jam_pulang - jam_datang) as total_detik FROM absensi WHERE user_id=%s AND EXTRACT(MONTH FROM tanggal) = %s AND EXTRACT(YEAR FROM tanggal) = %s ORDER BY tanggal", (user_id, bulan, tahun))
    data = cur.fetchall()
    conn.close()
    if not data:
    ...
    output.seek(0)
    await update.message.reply_document(document=InputFile(io.BytesIO(output.getvalue().encode()), filename=f"absen_{context.args[0]}.csv"), caption="📄 Data absen bulan ini")
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id!= ADMIN_ID:
        ...
        return
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("📋 Belum Absen Hari Ini", callback_data='admin_belum')], [InlineKeyboardButton("📅 Tambah Libur", callback_data='admin_libur')], [InlineKeyboardButton("❌ Tutup", callback_data='noop')]])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if button_id == 'pulang':
        if status not in ['datang', 'lembur']:
            ...
            return
        await query.answer("Silakan kirim lokasi untuk absen pulang", show_alert=True)
        context.user_data['aksi_absen'] = 'minta_lokasi_pulang'
        keyboard = ReplyKeyboardMarkup([[KeyboardButton("📍 Kirim Lokasi Saya", request_location=True)]], resize_keyboard=True, one_time_keyboard=True)
        await query.edit_message_text(f"📍 Kirim lokasi Anda.\nRadius maks {RADIUS_METER}m dari kantor", reply_markup=keyboard)
        return AWAITING_LOCATION
...
def main():
    ...
    conv_handler = ConversationHandler(entry_points=[CallbackQueryHandler(button_handler)], states={REASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, terima_alasan)], AWAITING_LOCATION: [MessageHandler(filters.LOCATION, terima_lokasi)]}, fallbacks=[])
